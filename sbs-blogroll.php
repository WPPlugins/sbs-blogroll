    ...
    function widget($args, $instance) {
        ...
        foreach($items as $item) {
            if($total_items < $max_items){
                ...
                if($show_date) {
                    $output .= ' style="list-style-type: none;  background: transparent url(' . $item['feedicon'] . ') no-repeat 0 1px;">';
                }
                else {
                    $output .= '>';
                }
                if($show_feedname and $display_style!='feed') {
                    $output .= '<span class="sbsblogroll-feedname"><a href="'.$item['feedlink'].'">'.$item['feedname'].'</a></span><br />';
                    if($show_date)
                        $output .= '';
                }
                $output .= '<a href="'.$item['link'].'" title="'.$item['description'].' '.$item['feedname'].' '.$item_date.'" target="_blank">'.$item['title'].'</a><br />';
                if($show_date) {
